                    ...
                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        <?php if( $show_dateadded && isset( $item['dateadded'] ) ): ?>
		                        <div class="dateadded-container">
									<span class="dateadded"><?php echo sprintf( __( 'Added on : %s', 'yit' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) )?></span>
		                        </div>
	                        <?php endif; ?>
	                        ...
	                        <div class="actions-container">
	                            ...
	                            <?php if( $show_add_to_cart && isset( $stock_status ) && $stock_status != 'Out' ): ?>
		                            <div class="add-to-cart-container" style="<?php echo ( $is_user_owner && $repeat_remove_button )  ? 'width: 50%' : 'width: 100%' ?>">
		                                <?php
		                        ...
		                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
			                        <div class="remove-from-wishlist-container" style="<?php echo ( $show_add_to_cart && isset( $stock_status ) && $stock_status != 'Out' )  ? 'width: 50%' : 'width: 100%' ?>" >
	                                    <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="btn btn-ghost remove_from_wishlist button" title="<?php _e( 'Remove this product', 'yit' ) ?>"><?php _e( 'Remove', 'yit' ) ?></a>
			                        </div>
	                            <?php endif; ?>
	                        </div>
                        </td>
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
		        <div class="custom-add-to-cart-button-container">
			        <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
				        <a href="<?php echo ( $additional_info ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>" class="btn btn-ghost button ask-an-estimate-button" <?php echo ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> ><?php _e( 'Ask for an estimate', 'yit' ) ?></a>
			        <?php endif; ?>

		            <?php if( $show_cb ) : ?>
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="btn btn-flat-red button alt" id="custom_add_to_cart"><?php _e( 'Add the selected products to the cart', 'yit' ) ?></a>
		            <?php endif; ?>
		        </div>
...
<?php if( $additional_info ): ?>
	<div id="ask_an_estimate_popup">
		<form action="<?php echo $ask_estimate_url ?>" method="post" class="wishlist-ask-an-estimate-popup">
			...
			<button class="btn btn-flat-red button ask-an-estimate-button ask-an-estimate-button-popup" ><?php _e( 'Ask for an estimate', 'yit' ) ?></button>
		</form>
	</div>
<?php endif; ?>