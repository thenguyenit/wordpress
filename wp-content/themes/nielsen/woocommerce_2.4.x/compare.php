        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr( $product_class ); ?>">
                    <a class="a-style-2" href="<?php echo esc_url( add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ) ?>" data-product_id="<?php echo $product->id; ?>"><span class="remove">x</span><?php _e( 'Remove', 'yit' ) ?> </a>
                </td>
            <?php endforeach ?>
        </tr>
